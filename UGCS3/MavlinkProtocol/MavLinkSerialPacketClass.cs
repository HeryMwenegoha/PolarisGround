    ...
    public class MavLinkSerialPacketClass
    {
        ...
        public MavLinkSerialPacketClass(System.IO.Ports.SerialPort Port)
        {
           ...
           // this.packetcount            = 0;
            ...
            Console.WriteLine("Mavlink Protocol Initialised.");
        }
        ...
        // This Follows the Mission Planner Implementation
        ...
        public byte[] readMavPackets()
        {
            ...
            lock (ReadLock)
            {
                ...
                while (_sp.IsOpen)
                {
                    try
                    {
                        ...
                        //while (_sp.IsOpen)
                        {
                           // Console.WriteLine(_sp.ReadByte());
                            //byte[] _buf = new byte[3]{1,2,3};
                            //_sp.Write(_buf, 0, 3);
                    ...
                    if (buffer[0] == 254 || buffer[0] == 'U' || buffer[0] == 253)
                    {
                        ...
                        {
#if VERSION0
                            // We have Received More than 5 Bytes
                            if (SYSTEM_ID != 0)
                            {
                                // SYstem ID Can Never Be 0.. Wont make sense, Actually it will, i just decided that it can never be one.
                                if (SYSTEM_ID != buffer[3])
                                {
                                    if (buffer[3] == '3' || buffer[4] == 'D')
                                    {
                                        // 3DR radio
                                        //Debug.WriteLine("MAVLInk: 3DR Radio Device (rssi)");
                                    }
                                    else
                                    {
                                        Debug.WriteLine("MAVLInk Bad Packet, not addressed, Received Sys {0} Comp {1} vs Sys {2} Comp {3}", buffer[3], buffer[4], SYSTEM_ID, (int)MAVLink.MAV_COMPONENT.MAV_COMP_ID_ALL);
                                        return;
                                    }
                                }
                            }
            ...
            {
                // calculates the extra bit of crc for v1.0
                ...
                try
                {
                    if ((buffer[0] == 'U' || buffer[0] == 254 || buffer[0] == 253) && buffer.Length >= buffer[1])
                    {
                        if (buffer[3] == '3' && buffer[4] == 'D')
                        {
                            ...
                            // wont accept packets from 3DR Radio
                        }
                        ...
                        else
                        {
                            ...
                            int expectedPacketMinus_one = ((recvpacketcount - 1) % 0x100);
                            {
                                if ((packetSeqNo != expectedPacketSeqNo) && (packetSeqNo != recvpacketcount) && (packetSeqNo != expectedPacketPlus_one)) // if sequence is not equal to expected as well as previous seqeunce: work around for duplicate packets
                                {
                                    ...
                                    if (!Variables.WAITING_FOR_PARAM_LIST && PROCEED)
                                    {
                                        numLost = 0;
                                    }
                                    PROCEED = Variables.WAITING_FOR_PARAM_LIST;

                                    // Put better antenna and lets see the results.
                                    Console.WriteLine("SEQ " + packetSeqNo + " ; " +"Expected " + expectedPacketSeqNo + " ; ID " + buffer[5]);
                                    ...
                                    //if (buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.RADIO)
                                    {
                                       ...
                                       // mavlink_radio_t = buffer.ByteArrayToStructure<MAVLink.mavlink_radio_t>(6);
                                    }

                                    packetsLost += numLost;
                                    //Console.WriteLine("PCKS LOST " + packetsLost);
                                    // Debug.WriteLine("Lost pksts new seqno {0} pks Lost {1}", packetSeqNo, numLost);
                                }
                        ...
                        if (buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.PARAM_VALUE)
                        {
                            ...
                            // mavlink_param_value_t = buffer.ByteArrayToStructure<MAVLink.mavlink_param_value_t>(6);
                            // HandParametersToEvent(ASCIIEncoding.ASCII.GetString(mavlink_param_value_t.param_id, 0, 15), mavlink_param_value_t.param_index, mavlink_param_value_t.param_count, mavlink_param_value_t.param_value);
                        }
                        */

                        /*
                        if (buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.ATTITUDE)
                        {
                        ...
                        if (buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.ATTITUDE)
                        {
                            MAVLink.mavlink_attitude_t mavlink_attitude_t;
                            mavlink_attitude_t = buffer.ByteArrayToStructure<MAVLink.mavlink_attitude_t>(6);
                            Console.WriteLine(mavlink_attitude_t.roll * 57.6);
                        }
                        */


                        /* Decode Different Messages
                        if (buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.HEARTBEAT)
                        {
                            mavlink_heartbeat_t = buffer.ByteArrayToStructure<MAVLink.mavlink_heartbeat_t>(6);
                        }
                        ...
                        if(buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.HIL_RC_INPUTS_RAW)
                        {
                            mavlink_hil_rc_inputs_raw_t = buffer.ByteArrayToStructure<MAVLink.mavlink_hil_rc_inputs_raw_t>(6);
                        }
                        ...
                        if(buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.VFR_HUD)
                        {
                            mavlink_vfr_hud_t = buffer.ByteArrayToStructure<MAVLink.mavlink_vfr_hud_t>(6);
                        }

                        // Decode GPS
                        if (buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.GPS_RAW_INT)
                        {
                            mavlink_gps_raw_int_t = buffer.ByteArrayToStructure<MAVLink.mavlink_gps_raw_int_t>(6);
                        }
                        if(buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.GLOBAL_POSITION_SETPOINT_INT)
                        {
                            mavlink_wp_setpoint =  buffer.ByteArrayToStructure<MAVLink.mavlink_global_position_setpoint_int_t>(6);
                        }

                        // Decode set_mode from uav rx|tx unit
                        if(buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.SET_MODE)
                        {
                            mavlink_set_mode_t = buffer.ByteArrayToStructure<MAVLink.mavlink_set_mode_t>(6);
                        }

                        // Decode System Status
                        if (buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.SYS_STATUS)
                        {
                            mavlink_sys_status_t = buffer.ByteArrayToStructure<MAVLink.mavlink_sys_status_t>(6);
                        }
                        ...
                        {
                            mavlink_waypoint_request_t = buffer.ByteArrayToStructure<MAVLink.mavlink_mission_request_t>(6);
                        }

                        if (buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.RC_CHANNELS_RAW)
                        {
                            mavlink_rc_channels_raw_t = buffer.ByteArrayToStructure<MAVLink.mavlink_rc_channels_raw_t>(6);
                        }

                        // Receive Mission Item/ WayPoint
                        if(buffer[5] ==(byte)MAVLink.MAVLINK_MSG_ID.MISSION_ITEM)
                        {
                            mavlink_mission_item_t = buffer.ByteArrayToStructure<MAVLink.mavlink_mission_item_t>(6);
                        }

                        // mission_ack
                        if(buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.MISSION_ACK)
                        {
                            mavlink_mission_ack_t = buffer.ByteArrayToStructure<MAVLink.mavlink_mission_ack_t>(6);
                        }

                        // mission count
                        if(buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.MISSION_COUNT)
                        {
                            mavlink_mission_count_t_rx = buffer.ByteArrayToStructure<MAVLink.mavlink_mission_count_t>(6);
                        }


                        // status text
                        if(buffer[5] == (byte)MAVLink.MAVLINK_MSG_ID.STATUSTEXT)
                        {
                            mavlink_status_text_t = buffer.ByteArrayToStructure<MAVLink.mavlink_statustext_t>(6);
                            string P = ASCIIEncoding.ASCII.GetString(mavlink_status_text_t.text, 0, 50);
                            HandStatusTextToEvent(P);
                            //Debug.WriteLine("STATUS:" + mavlink_status_text_t.text.ToString());
                        }
                        */
            ...
            //Console.WriteLine(sw_watch.ElapsedMilliseconds);
        }
